...
const char * const BITCOIN_CONF_FILENAME = "soori.conf";
const char * const BITCOIN_PID_FILENAME = "soorid.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "soori" is a composite category enabling all Soori-related debug output
            if(ptrCategory->count(string("soori"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SooriCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SooriCore
    // Mac: ~/Library/Application Support/SooriCore
    // Unix: ~/.sooricore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SooriCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SooriCore";
#else
    // Unix
    return pathRet / ".sooricore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty soori.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override soori.conf
